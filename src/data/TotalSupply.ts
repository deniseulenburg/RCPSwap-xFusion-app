...
export function useTotalSupply(token?: Token, chainId?: ChainId): TokenAmount | undefined {
  const contract = useTokenContract(token?.address, false)

  const totalSupply: BigNumber = useSingleCallResult(contract, 'totalSupply', undefined, chainId)?.result?.[0]
