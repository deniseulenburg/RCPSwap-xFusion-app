...
export abstract class BaseSwapping {
        ...
        if (this.transit.isV2()) {
            ...
            if (feeV2) {
                if (JSBI.lessThan(amount, feeV2.raw)) {
                    throw new Error(
                        `Amount ${this.transit.amountOut.toSignificant()} ${feeV2.token.symbol
    ...
    protected buildTradeA(): SymbiosisTrade {
        ...
        if (XfusionTrade.isAvailable(chainId)) {
            ...
            return new XfusionTrade(this.tokenAmountIn, tokenOut, to, this.slippage['A'], routerA)
        }
    ...
    protected buildTradeC(feeV2?: TokenAmount) {
        ...
        if (this.transit.isV2()) {
            ...
            if (feeV2) {
                if (amountIn.lessThan(feeV2)) {
                    throw new Error(
                        `Amount ${amountIn.toSignificant()} ${amountIn.token.symbol
                        } less than fee ${feeV2.toSignificant()} ${feeV2.token.symbol}`,
                        ErrorCode.AMOUNT_LESS_THAN_FEE
                    )
                }
        ...
        if (XfusionTrade.isAvailable(chainId)) {
            ...
            return new XfusionTrade(amountIn, this.tokenOut, this.tradeCTo(), this.slippage['C'], routerC)
        }
