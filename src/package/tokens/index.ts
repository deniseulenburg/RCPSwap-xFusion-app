...
import { currencyFromShortCurrencyName, isShortCurrencyName, isShortCurrencyNameSupported, Token } from '../currency'
...
async function populateCache() {
  ...
  for (const chainId of chainIds) {
    const tokens = z
      .array(tokenSchema)
      .parse(await fetch(`https://tokens-git-feature-swap.sushi.com/v0/${chainId}`).then((data: any) => data.json()))
...
async function fetcher(chainId: ChainId, tokenId: string) {
  ...
  const token = tokenSchema.parse(
    await fetch(`https://tokens-git-feature-swap.sushi.com/v0/${chainId}/${getAddress(tokenId)}`).then((data: any) =>
      data.json()
    )
  )
...
export async function getToken(chainId: ChainId, tokenId: string) {
  ...
  if (!isAddress(tokenId)) return null
